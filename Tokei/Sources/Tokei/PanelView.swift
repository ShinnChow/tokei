...
struct PanelView: View {
    ...
    private var visibleCount: Int {
        [showClaude, showCodex, showGemini, showCursor, showZed, showSub2API, showZai,
         ...
         showQwenWork, showKimiCode, showCmdCode, showPrimeAgent].filter { $0 }.count
    }
    ...
    private func toolCards(for u: Usage) -> [ToolCardItem] {
        ...
        let ccr = u.cmdcode.ranges.get(sel)
        return [
        ...
        if let data = result.stdout.data(using: .utf8),
           let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            let tools = ["claude", "codex", "gemini", "antigravity", "cursor", "zed",
                         ...
                         "deepseek_harness",
                         "opencode", "qwencode", "qwenwork", "kimicode", "cmdcode", "prime_agent"]
                .filter { json[$0] != nil }
                .joined(separator: ",")
